...
# st.set_page_config(page_title='None', page_icon=None, initial_sidebar_state="auto")

st.set_page_config(layout='wide', page_title='Luna Travel',
                   page_icon='./assets/LunaLogo.png')
...
def main():
    ...
    if nmuser and gsenha and vLogin:
        ...
        st.session_state['login'] = vLogin[4]

        # st.sidebar.write(sStore)

        det_store_result = sStore

        if det_store_result == None:
            st.sidebar.write('Connection problem!')
        else:
            st.session_state['nmstore'] = storeC.get_det_store(sStore[1])[1]
        ...
        if not loginOk:
            ...
            if (nmuser == 'admin') and (loginOk) and (sStore == None):
                st.sidebar.warning(
                    'Unregistered computer. Ask the administrator to register the computer!')
            else:
                st.session_state['idstore'] = storeC.get_det_store(sStore[1])[
                    0]
            ...
            st.sidebar.divider()
            if (sStore != None):
                st.sidebar.success(st.session_state.nmstore)
            else:
                if (nmuser != "admin"):
                    st.sidebar.error('User does not exist!')
    ...
    # st.sidebar.write(sys.version[:6])
