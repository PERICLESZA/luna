...
def add_new_exchange():
    ...
    pnCol1, pnCol2, pnCol3, pnCol4, pnCol5, pnCol6, pnCol7  = st.columns([1,1,1,1,1,1,1])
    with pnCol1:
        ...
        if sel_item_id: Exchange.fk_idcustomer = sel_item_id[0]

        items = bankC.get_all_bank(1)
        x = lambda x: x[1]
        sel_item_id = sty.overlaid_selectbox('Bank', items, 0, x)
        if sel_item_id: Bank.fk_idcustomer = sel_item_id[0]
...
def mainExchange():
    ...
    selected_i_id = st.selectbox("Phone No./Name", items, format_func=lambda x: x[1])
    st.write('')
